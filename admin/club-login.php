<?php
/**
 * Club Leadership Dedicated Login Portal
 */
    ...
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($email) || empty($password)) {
            ...
            if ($user && password_verify($password, $user['password_hash'])) {
                ...
                $error = "Invalid club email or password. If you are Dean Sir, please use the Dean Portal login.";
            }
        }
    }
}
    ...
    <div class="row justify-content-center py-4">
        <div class="col-md-5 col-lg-4">
            <div class="card p-4 p-md-5 border-0 shadow-lg rounded-4 text-center">
                ...
                <p class="text-secondary small mb-4">President & Core Team Management</p>

                <?php if (!empty($error)): ?>
                    <div class="alert alert-danger rounded-3 small mb-3 text-start"><i class="bi bi-exclamation-circle-fill me-1"></i> <?= e($error) ?></div>
                <?php endif; ?>

                <form action="/admin/club-login.php" method="POST" class="text-start">
                ...
                <div class="pt-3 border-top mt-3 small text-muted">
                    ...
                    <a href="/admin/dean-login.php" class="fw-bold text-primary text-decoration-none ms-1">Go to Dean Sir Login &rarr;</a>
                </div>
            </div>
        </div>
    </div>
